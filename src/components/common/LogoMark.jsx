...
// Just the hexagonal "BS" monogram mark — no wrapping link, no wordmark.
// Extracted out of Logo.jsx so it can be reused anywhere a plain visual
// mark is needed (e.g. the Preloader) without dragging along Logo's
// site-navigation <a href="#home"> semantics.
//
// Gradient ids are unique per instance (useId) so multiple simultaneous
// copies on the page (nav + mobile drawer + footer, etc.) don't collide —
// SVG url(#id) references resolve against the whole document, not scoped
// to the nearest <svg>.
const LogoMark = ({ className = '' }) => {
  const uid = useId();
  const goldGradientId = `logoGoldGradient-${uid}`;
  const goldLightGradId = `logoGoldLightGrad-${uid}`;
  ...
  return (
    <svg viewBox="0 0 100 100" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      />
      {/* Central B Bar */}
      <path d="M28 46 H52" stroke={`url(#${goldGradientId})`} strokeWidth="5" strokeLinecap="square" />
